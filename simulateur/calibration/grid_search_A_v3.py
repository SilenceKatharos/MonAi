"""
Grid search v3 — implementation of Avenue P2: R_acc as a direct multiplier
on PoUW reward (cf. formalisation/01 §C.0).
...
w_acc fixed at 0.05 (symbolic role in aggregated R; the main economic
penalty is carried by the direct multiplier in reward).

Output: simulateur/calibration/grid_search_A_v3.csv
"""
...
W_ACC_FIXED = 0.05  # symbolic — the main penalty is carried by the multiplier
...
def main() -> None:
    ...
    out_csv = os.path.join(base, "simulateur", "calibration", "grid_search_A_v3.csv")
    ...
    print(f"[grid v3] Launching {n_total} runs (R_acc multiplier, w_acc={W_ACC_FIXED} fixed)...")

    for idx, (f0_max, k_sig, gamma) in enumerate(combos, start=1):
        cfg = make_scenario_A_config(f0_max, k_sig, gamma)
        scen_tag = f"grid_A_v3_f{f0_max}_k{int(k_sig)}_g{gamma}"
    ...
    total = time.time() - t0
    print(f"\n[grid v3] Done in {total:.0f}s ({total/60:.1f} min). Results: {out_csv}")
